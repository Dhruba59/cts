...
import {
  useAddUser,
  useEditUser,
  useGetSiteDetailWithProtocol,
  useGetUserById,
  useGetUserDropdowns,
  useValidateUserName
} from "@/hooks/rq-hooks/user-hooks";
...
const constructPayload = (
  ...
  switch (userType) {
    case USER_TYPE_ENUM.SITE_USER:
      payload.protocolIds = values?.protocols
        ?.map((item: DndDataItem) => item.value)
        .join(",");
      payload.siteId = values?.site?.value ?? values?.site ?? undefined;
      payload.suppressMatchTypeId =
        values?.suppressMatchType?.value ??
        values?.suppressMatchType ??
        undefined;
      payload.trainings = values?.training?.map((item: any) => ({
        trainingId: item?.value,
    ...
    case USER_TYPE_ENUM.SYS_ADMIN:
      payload.matchTypeIds = values?.matchTypes
        ?.map((item: DndDataItem) => item.value)
        .join(",");
      payload.notificationSiteIds = values?.notificationSites
        ?.map((item: DndDataItem) => item.value)
        .join(",");
      break;
    case USER_TYPE_ENUM.SPONSOR:
      payload.sponsorProtocols = values?.protocols
        ?.map((item: DndDataItem) => item.value)
        .join(",");
      break;
...
const AddUser = ({ id }: AddUserProps) => {
  ...
  const [selectedUserType, setSelectedUserType] = useState<USER_TYPE_ENUM>(
    USER_TYPE_ENUM.SITE_USER
  );
  const [selectedSponsorId, setSelectedSponsorId] = useState<string>("");
  const [initialSponsorProtocolIds, setInitialSponsorProtocolIds] =
    useState<string[]>();
  const [initialSiteProtocolIds, setInitialSiteProtocolIds] =
    useState<string[]>();
  ...
  const { data: dropdowns, isLoading: isDropdownDataLoading } =
    useGetUserDropdowns();
  ...
  const [siteUserDndData, setSiteUserDndData] = useState<DndDataType[]>(
    initialSiteUserDndValue
  );
  const [trainingDndData, setTrainingDndData] = useState<DndDataType[]>(
    initialTrainingDndValue
  );
  const [sponsorDndData, setSponsorDndData] = useState<DndDataType[]>(
    initialSponsorDndValue
  );
  const [adminDndData, setAdminDndData] =
    useState<AdminDndData>(initialAdminDndValue);
  const storeSetDndData = useProtocolListStore((state) => state.setDndData);
  const storeSetSiteDetail = useProtocolListStore(
    (state) => state.setSiteDetail
  );
  const storeSetInitialSiteProtocolIds = useProtocolListStore(
    (state) => state.setInitialSiteProtocolIds
  );
  const { data: siteDetail, refetch: refetchSiteDetailWithProtocol } =
    useGetSiteDetailWithProtocol({ SiteId: siteUserSiteId });
  storeSetSiteDetail(siteDetail);
  const storeSetAdminDndData = useProtocolListStore(
    (state) => state.setAdminDndData
  );
  const storeSetSelectedProtocols = useProtocolListStore(
    (state) => state.setSelectedProtocols
  );
  ...
  const updateFieldsWithUserData = (user: any) => {
    ...
    if (user) {
      ...
      if (user.userTypeId == USER_TYPE_ENUM.SITE_USER) {
        // setSiteUserDndData();
        setInitialSiteProtocolIds(
          user?.protocolIds?.map((id: number) => id.toString())
        );
        ...
        const adminMatchTypes = searchByIds(
          dropdowns?.data?.matchTypes,
          user?.matchTypeIds
        );
        const adminNotificationSites = searchByIds(
          dropdowns?.data?.sites,
          user?.notificationSiteIds
        );
    ...
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6 px-6 py-8">
            ...
            <div>
              ...
              {errors.systemLogin && (
                <span
                  className={`${
                    errors.systemLogin.type === "custom"
                      ? "text-green-500"
                      : "text-red-500"
                  } -mt-10`}
                >
                  {errors.systemLogin.message as string}
                </span>
              )}
            </div>
            <div>
            ...
            <div>
              <Textarea
                ...
                className="min-h-10 h-10 rounded-sm"
              />
              {errors.address1 && (
                <span className="text-red-500 -mt-10">
        ...
        <section className="wrapper space-y-6">
          <Tab
            tabItems={getTabItems(selectedUserType)}
            titleClassname="my-2 px-2"
          />
        </section>
